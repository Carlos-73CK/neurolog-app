...
function ChildrenGrid({ isLoading, error, filteredChildren, allChildren, handlers }: any) {
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[...Array(6)].map((_, i) => (
          // --- CORRECCIÓN: Usar una key más estable que el índice ---
          <Card key={`skeleton-loader-${i}`} className="animate-pulse">
            <CardHeader>
  ...
  if (filteredChildren.length === 0) {
    ...
    // Si hay niños pero los filtros no arrojan resultados
    return (
      <Card>
        <CardContent className="text-center py-12">
          <UsersIcon className="mx-auto h-12 w-12 text-gray-300 mb-4" />
...
export default function ChildrenPage() {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid'); // Estado para el modo de vista
